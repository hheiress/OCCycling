...
function update(req, res) {
    const { bike_id, user_id, last_name, status, renting_date, station_id, time_left, conditions_id } = req.body;
    const { id } = req.params;
    if (!bike_id || !user_id || !last_name || !status || !renting_date || !station_id || !time_left || !conditions_id) {
        return res
            .status(400)
            .send("Please insert a bike id, user id, last_name, status, renting date, station id, time left, conditions");
    }
    return pool
        .query("update rentings set bike_id = $2, user_id = $3, last_name=4	status = $5 renting_date =$6, station_id = $7, time_left = $8, conditions_id = $9 WHERE id = $1", [id, bike_id, user_id, last_name, status, renting_date, station_id, time_left, conditions_id])
